               ...
               contin_t min_contin, contin_t max_contin)
{
	...
	for (int i = 0; i < nsamples; ++i) {
		...
		for (auto it = root.begin(); it != root.last_child(); ++it)
			...
			else OC_ASSERT(false, "Not implemented yet");
...
contin_t OTable::abs_distance(const OTable &ot) const
{
	...
	} else if (id::boolean_type == type and id::boolean_type == ot.type) {
		for (const_iterator x = begin(), y = ot.begin(); x != end();)
			res += (contin_t) (get_builtin(*(x++)) != get_builtin(*(y++)));
	} else if (id::enum_type == type and id::enum_type == ot.type) {
		for (const_iterator x = begin(), y = ot.begin(); x != end();)
...
void subsampleCTable(float ratio, CTable &ctable)
{
	...
	dorepeat(nremove)rm_row_idxs.insert(rm_selector());
